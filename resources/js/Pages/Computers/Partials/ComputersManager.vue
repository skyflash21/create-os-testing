...
const confirmDelete = () => {
    deleteForm.delete(route("computers.destroy", form.id), {
        onSuccess: () => {
            ...
            props.computers = props.computers.filter(
                (computer) => computer.computer_id !== form.id
            );
            form.reset();
...
onMounted(() => {
    window.addEventListener("computer_list", (event) => {
        ...
        connectedComputers.value = $computerList
            .filter((computer) => !computer.isUser)
    ...
    window.addEventListener("computer_remove", (event) => {
        ...
        // if the removed computer is the one with the opened terminal, close the terminal
        if (
            selectedComputer.value &&
            selectedComputer.value.computer_id === event.detail.id
        ) {
            selectedComputer.value = null;
            showTerminal.value = false;

            // notify the user that the computer has been disconnected
    ...
    <div class="flex h-screen p-6 bg-gray-900">
        ...
        <div class="w-1/2 p-2 border-r bg-gray-800 rounded-lg">
            ...
            >
                ...
                >
                    <!-- Image de l'ordinateur, avancé ou non -->
                    <img
                        :src="
        ...
        <div class="w-2/3 p-4 bg-gray-800 rounded-lg detail-container">
                ...
                <div class="mb-6 text-white">
                    <div class="flex justify-between">
                        <div>
                            ...
                            <p class="text-sm font-medium">
                                Nom: {{ selectedComputer.computer_name }}
                            </p>
                            <p class="text-sm font-medium">
                                Description:
                                {{ selectedComputer.computer_description }}
                            </p>
                            <p class="text-sm font-medium">
                                Type: {{ selectedComputer.type }}
                            </p>
                            <p class="text-sm font-medium">
                                Avancé:
                                {{
                                    selectedComputer.is_advanced ? "Oui" : "Non"
                                }}
                            </p>
                            <p class="text-sm font-medium">
                                Côté Modem Sans Fil:
                                {{ selectedComputer.wireless_modem_side }}
                            </p>
                            <p class="text-sm font-medium">
                                Dernière Utilisation:
                                {{
                                    new Date(
                                        selectedComputer.last_used_at
                                    ).toLocaleDateString()
                                }}
                            </p>
                            <p class="text-sm font-medium">
                    ...
                    <div class="mt-6">
                        <p class="text-lg font-medium">
                            ...
                            {{ selectedComputer.used_disk_space }} /
                            {{ selectedComputer.total_disk_space }} bytes
                        </p>
                        <div class="h-4 bg-gray-300 rounded">
                            <div
                                ...
                                :style="{
                                    width:
                                        (selectedComputer.used_disk_space /
                                            selectedComputer.total_disk_space) *
                                            100 +
                                        '%',
                                }"
                            ></div>
            ...
            <div v-else class="text-white">
                <p>
                    Aucun ordinateur sélectionné. Cliquez sur un ordinateur dans
                    la liste pour voir les détails.
                </p>
            </div>
        </div>
        ...
        >
            ...
            <template #content>
                <p>
                    Êtes-vous sûr de vouloir supprimer cet ordinateur ? Cette
